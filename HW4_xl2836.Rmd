...
By using function `confint`, we get 95% CIs of all estimators. The 95% CIs of `severity` is (-1.4348, 0.5508) which means at $\alpha = 0.05$ significant level, we can conclude that the true coefficient of `severity` is between -1.4348 and 0.5508. The estimated coefficient of `severity` is -0.442 which means if the value of `severity` increased by 1 units, the value of satisfaction will decrease 0.442 given all other values of predictors stay constant.
